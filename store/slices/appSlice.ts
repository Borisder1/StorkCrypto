...
export const createAppSlice: StateCreator<StoreState, [], [], AppSlice> = (set, get) => ({
    ...
    updateSentinelConfig: async (config) => {
        ...
        // Sync Sentinel to DB for Backend access
        const state = get();
        if (state.userStats.id !== 'INIT' && !state.userStats.id.startsWith('GUEST')) {
            await supabase.from('profiles').update({ 
                sentinel_config: state.userStats.sentinel 
            }).eq('id', state.userStats.id);
        }
    },
        ...
        try {
            ...
            if (profile) {
                set(state => ({ 
                    userStats: { 
                        ...
                        username: tgUser?.username || profile.username || state.userStats.username,
                        avatarUrl: tgUser?.photo_url || localStorage.getItem('stork_user_avatar') || undefined,
                        xp: Number(profile.xp) || 0,
                        storkBalance: Number(profile.stork_balance) || 0,
                        subscriptionTier: (profile.subscription_tier as any) || 'FREE',
                        sentinel: profile.sentinel_config || state.userStats.sentinel
                    } 
                }));
            } else {
                ...
                set(state => ({ 
                    userStats: { 
                        ...state.userStats, 
                        id,
                        firstName: tgUser?.first_name || 'OPERATOR',
                        username: tgUser?.username || '',
                        avatarUrl: tgUser?.photo_url || localStorage.getItem('stork_user_avatar') || undefined,
                    } 
                }));
            ...
            set(state => ({ 
                userStats: { 
                    ...state.userStats, 
                    id,
                    username: tgUser?.username || state.userStats.username,
                    avatarUrl: tgUser?.photo_url || localStorage.getItem('stork_user_avatar') || state.userStats.avatarUrl
                } 
            }));
        } finally {
