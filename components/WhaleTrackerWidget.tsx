...
export const WhaleTrackerWidget: React.FC = () => {
        ...
        const unsubscribe = binanceWS.subscribeTrades((trade) => {
            ...
            if (valueUsd >= WHALE_THRESHOLD_USD) {
                ...
                const newTx: WhaleTransaction = {
                    ...
                    aiComment: valueUsd > 500000 ? 'whale.mega_block' : (isBuyerMaker ? 'whale.retail_dist' : 'whale.heavy_accum')
                };
    ...
    return (
        <div className="relative mb-3 group">
            ...
            <div className="bg-[#0a0f1e]/80 backdrop-blur-sm border border-white/5 p-3 relative overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-3 relative z-10">
                    <div className="flex items-center gap-3">
                        <div className="w-6 h-6 flex items-center justify-center rounded bg-brand-cyan/10 border border-brand-cyan/30">
                            <RadarIcon className="w-3 h-3 text-brand-cyan animate-[spin_4s_linear_infinite]" />
                        </div>
                        <div>
                ...
                <div className="space-y-1 relative z-10">
                    {recentTxs.length === 0 ? (
                        <div className="flex items-center justify-center py-6 border border-dashed border-white/10 bg-black/20">
                            <p className="text-[9px] font-mono text-slate-500 uppercase tracking-widest animate-pulse">{t('whale.scanning')}</p>
                        </div>
                    ) : (
                        recentTxs.map(tx => (
                            <div key={tx.id} className="flex items-center justify-between p-2 hover:bg-white/5 transition-colors border-l-2 border-transparent hover:border-brand-cyan pl-2">
                                ...
                                <div className="text-right">
                                    <div className={`text-[10px] font-black font-mono ${tx.type === 'EXCHANGE_INFLOW' ? 'text-red-400' : 'text-green-400'}`}>
                                        {tx.type === 'EXCHANGE_INFLOW' ? t('whale.dump') : t('whale.pump')}
                                    </div>
