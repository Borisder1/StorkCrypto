            ...
            <div className="relative z-10 w-full sm:max-w-md bg-brand-bg border-t sm:border border-white/10 rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] animate-slide-up-mobile">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Push Alerts Configuration</h4>
                        
                        {/* PACKAGE ALERT */}
                        <div className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${config.notifyExpiringPack ? 'bg-brand-purple/10 border-brand-purple/30' : 'bg-white/5 border-white/5 opacity-60'}`} onClick={() => toggle('notifyExpiringPack')}>
                            <div className="flex items-center gap-3">
                                <TimerIcon className={`w-5 h-5 ${config.notifyExpiringPack ? 'text-brand-purple' : 'text-slate-500'}`} />
                                <div>
                                    <p className="text-xs font-bold text-white">Subscription Expiry</p>
                                    <p className="text-[8px] text-slate-500 font-mono">Notify 24h before end</p>
                                </div>
                            </div>
                        ...
                        {/* BREAKING NEWS ALERT */}
                        <div className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${config.notifyBreakingNews ? 'bg-brand-cyan/10 border-brand-cyan/30' : 'bg-white/5 border-white/5 opacity-60'}`} onClick={() => toggle('notifyBreakingNews')}>
                            <div className="flex items-center gap-3">
                                <NewspaperIcon className={`w-5 h-5 ${config.notifyBreakingNews ? 'text-brand-cyan' : 'text-slate-500'}`} />
                                <div>
                                    <p className="text-xs font-bold text-white">Breaking Intel</p>
                                    <p className="text-[8px] text-slate-500 font-mono">Push only major news</p>
                                </div>
                            </div>
                        ...
                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-4">Market Watch Parameters</h4>
