            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-brand-card/60 border border-white/10 rounded-2xl p-4">
                                    <div className="flex items-center justify-between gap-1.5 mb-1">
                                        <p className="text-[8px] text-slate-500 font-black uppercase">Fractal_Dimension (FDI)</p>
                                        <HelpIndicator id="asset_chart" />
                                    </div>
                    ...
                    {activeTab === 'ORDERBOOK' && (
                        <div className="space-y-6 animate-fade-in w-full">
                            <div className="bg-brand-card/40 border border-white/5 rounded-[2.5rem] p-6">
                                <h3 className="text-[10px] text-brand-cyan font-black uppercase mb-4 flex items-center gap-2"><ActivityIcon className="w-4 h-4"/> Market_Depth (Orderbook) <HelpIndicator id="asset_orderbook" /></h3>
                                {!orderBook ? (
                    ...
                    {activeTab === 'WATCHDOG' && (
                        <div className="space-y-6 animate-fade-in">
                            <div className="bg-brand-cyan/5 border border-brand-cyan/20 rounded-[2.5rem] p-6">
                                <h3 className="text-[10px] text-brand-cyan font-black uppercase mb-4 flex items-center gap-2"><BellIcon className="w-4 h-4"/> Deploy_Price_Sentinel <HelpIndicator id="asset_watchdog" /></h3>
                                <div className="flex gap-2">
                    ...
                    {activeTab === 'BACKTEST' && (
                        <div className="space-y-6 animate-fade-in w-full">
                            <div className="bg-brand-card/40 border border-white/5 rounded-[2.5rem] p-6">
                                <h3 className="text-[10px] text-brand-cyan font-black uppercase mb-4 flex items-center gap-2"><ActivityIcon className="w-4 h-4"/> Strategy_Backtest_Engine <HelpIndicator id="asset_backtest" /></h3>
                    ...
                    {activeTab === 'AI_SETUP' && (
                        <div className="space-y-6 animate-fade-in">
                            <div className="bg-brand-purple/5 border border-brand-purple/20 rounded-[2.5rem] p-8 relative overflow-hidden">
                                <h4 className="text-[10px] text-brand-purple font-black uppercase mb-6 flex items-center gap-2"><BotIcon className="w-5 h-5"/> Live_Neural_Synthetics <HelpIndicator id="asset_ai_simulation" /></h4>
                                {aiLoading ? <div className="text-xs text-slate-500 animate-pulse font-mono">📡 LINKING_TO_GLOBAL_SEARCH...</div> : (
                                    <div className="space-y-4">
                                        <p className="text-sm text-slate-200 font-mono leading-relaxed italic">
                                            "{aiReport || (settings.language === 'ua' ? 'Технічний аналіз тимчасово оновлюється. Очікуйте синхронізації з блокчейн-вектором.' : settings.language === 'pl' ? 'Analiza techniczna jest tymczasowo aktualizowana. Czekaj na synchronizację sygnałów.' : 'Technical analysis matrix momentarily recycling. Waiting for system vector lock...')}"
                                        </p>
