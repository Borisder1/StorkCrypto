...
export const ChartPattern: React.FC<{ type: VisualType }> = ({ type }) => {
    ...
    return (
        <div className="h-32 sm:h-40 w-full bg-brand-card/20 backdrop-blur-sm rounded-xl border border-white/10 mb-3 flex items-center justify-center relative overflow-hidden group hover:bg-brand-card/30 hover:border-white/20 transition-all duration-300 hover:shadow-lg">
            {/* Grid Background */}
            ...
            {/* Scanline */}
            <div className="absolute top-0 w-full h-1 bg-gradient-to-r from-transparent via-brand-cyan/30 to-transparent animate-[scanline_3s_linear_infinite] opacity-60"></div>
            ...
            <style>{`
                ...
                .chart-element {
                    animation: chart-glow 2s infinite;
                }
            `}</style>

            {type === 'CHART_HEAD_SHOULDERS' && (
                <svg viewBox="0 0 200 100" className="w-full h-full p-2 sm:p-4 stroke-brand-cyan fill-none stroke-[2px] chart-element">
                    ...
                    <text x="80" y="15" fill="#00F0FF" stroke="none" fontSize="8" fontWeight="bold" className="chart-element">HEAD</text>
            ...
            {type === 'CHART_DOUBLE_TOP' && (
                <svg viewBox="0 0 200 100" className="w-full h-full p-2 sm:p-4 stroke-brand-danger fill-none stroke-[2px] chart-element">
                    <path d="M20,80 L50,20 L80,70 L110,20 L140,80" className="draw-path"/>
                    <line x1="20" y1="20" x2="140" y2="20" stroke="rgba(255,255,255,0.3)" strokeDasharray="4" />
                    <text x="45" y="15" fill="#ef4444" stroke="none" fontSize="8" fontWeight="bold" className="chart-element">RESISTANCE</text>
                </svg>
            )}

            {type === 'CHART_DOUBLE_BOTTOM' && (
                <svg viewBox="0 0 200 100" className="w-full h-full p-2 sm:p-4 stroke-brand-success fill-none stroke-[2px] chart-element">
                    <path d="M20,20 L50,80 L80,40 L110,80 L140,20" className="draw-path"/>
                    <line x1="20" y1="80" x2="140" y2="80" stroke="rgba(255,255,255,0.3)" strokeDasharray="4" />
                    <text x="45" y="92" fill="#22c55e" stroke="none" fontSize="8" fontWeight="bold" className="chart-element">SUPPORT</text>
                </svg>
            )}
            ...
            {type === 'CHART_BULL_FLAG' && (
                <svg viewBox="0 0 200 100" className="w-full h-full p-2 sm:p-4 stroke-brand-success fill-none stroke-[2px] chart-element">
                    ...
                    <text x="25" y="25" fill="#22c55e" stroke="none" fontSize="8" fontWeight="bold" className="chart-element">BREAKOUT</text>
                </svg>
            )}
            ...
            {type === 'CHART_WEDGE_BULL' && (
                <svg viewBox="0 0 200 100" className="w-full h-full p-2 sm:p-4 stroke-brand-success fill-none stroke-[2px] chart-element">
                    ...
                    <path d="M20,80 L40,50 L60,30 L80,60 L100,50 L120,70" className="draw-path" />
            ...
            {type === 'CANDLE_DOJI' && (
                <svg viewBox="0 0 100 100" className="w-full h-full p-2 sm:p-4">
                    <line x1="50" y1="10" x2="50" y2="90" stroke="white" strokeWidth="2" className="chart-element" />
