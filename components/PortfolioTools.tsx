    ...
    return (
        <div className="bg-brand-card/60 backdrop-blur-xl border border-white/10 rounded-[2rem] p-5 mb-6 overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.3)] mx-2">
            ...
            <div className="flex gap-2 mb-6 bg-black/40 p-1.5 rounded-2xl overflow-x-auto no-scrollbar border border-white/5">
                {[
                    { id: 'SHIELD', label: t('tools.shield'), color: 'bg-brand-cyan text-black' },
                    { id: 'TIME_MACHINE', label: t('tools.time_leap'), color: 'bg-brand-cyan text-black' },
                    { id: 'IL_CALC', label: t('tools.risk'), color: 'bg-brand-purple text-white' },
                    { id: 'TAX_EST', label: t('tools.tax'), color: 'bg-brand-green text-black' }
                ].map(tool => (
                    <button 
                        key={tool.id}
                ...
                <div className="animate-fade-in">
                    ...
                    <div className="bg-black/30 rounded-2xl p-4 border border-white/5 mb-4">
                        <div className="flex justify-between items-center mb-3">
                            <span className="text-[10px] text-slate-400 font-mono">{t('tools.contract_approvals')}</span>
                            {scanStatus === 'SCANNING' && <span className="text-[10px] text-yellow-500 animate-pulse font-bold">{t('tools.scanning')}</span>}
                            {scanStatus === 'SAFE' && <span className="text-[10px] text-green-500 font-bold drop-shadow-[0_0_5px_#00FF9D]">{t('tools.all_clear')}</span>}
                            {scanStatus === 'RISK' && <span className="text-[10px] text-red-500 font-bold">{t('tools.risk_found')}</span>}
                            {scanStatus === 'IDLE' && <span className="text-[10px] text-slate-600">{t('tools.last_scan')}</span>}
                        </div>
                    ...
                    <div className="flex justify-center">
                        ...
                        >
                            {scanStatus === 'SAFE' ? t('tools.system_secure') : t('tools.scan_permissions')}
                        </button>
                    </div>
            ...
            {activeTool === 'TIME_MACHINE' && (
                <div className="animate-fade-in">
                    <LockedFeature minLevel={5} title={t('tools.fomo_simulator')}>
                        <div className="flex items-center gap-2 mb-3">
                            <BotIcon className="w-4 h-4 text-brand-cyan" />
                            <span className="text-xs font-bold text-white tracking-widest font-orbitron">{t('tools.fomo_simulator')}</span>
                        </div>
                        ...
                        {tmResult !== null && (
                            <div className="bg-brand-cyan/10 border border-brand-cyan/30 rounded-xl p-3 mb-3 text-center">
                                <p className="text-[9px] text-slate-400 uppercase font-mono">{t('tools.projected_value')}</p>
                        ...
                        <button onClick={calculateTimeMachine} className="w-full py-3 bg-white/5 border border-white/10 hover:bg-brand-cyan/20 hover:border-brand-cyan hover:text-brand-cyan transition-all rounded-xl text-[10px] font-bold uppercase tracking-widest">{t('tools.calculate_potential')}</button>
                    </LockedFeature>
                </div>
            )}
            ...
            {activeTool === 'IL_CALC' && (
                <div className="animate-fade-in">
                    <LockedFeature minLevel={10} title={t('tools.impermanent_loss')}>
                        <div className="flex items-center gap-2 mb-3">
                            <BarChartIcon className="w-4 h-4 text-brand-purple" />
                            <span className="text-xs font-bold text-white tracking-widest font-orbitron">{t('tools.impermanent_loss')}</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mb-3">
                            <div>
                                <label className="text-[8px] text-slate-500 uppercase block mb-1">{t('tools.token_a_change')}</label>
                            ...
                            <div>
                                <label className="text-[8px] text-slate-500 uppercase block mb-1">{t('tools.token_b_change')}</label>
                        ...
                        {ilResult !== null && (
                            <div className="bg-brand-purple/10 border border-brand-purple/30 rounded-xl p-3 mb-3 text-center">
                                <p className="text-[9px] text-slate-400 uppercase font-mono">{t('tools.est_loss')}</p>
                                <p className="text-xl font-black text-brand-purple font-orbitron drop-shadow-[0_0_8px_#BD00FF]">{ilResult.toFixed(2)}%</p>
                            </div>
                        )}

                        <button onClick={calculateIL} className="w-full py-3 bg-white/5 border border-white/10 hover:bg-brand-purple/20 hover:border-brand-purple hover:text-brand-purple transition-all rounded-xl text-[10px] font-bold uppercase tracking-widest">{t('tools.check_risk')}</button>
                    </LockedFeature>
                </div>
            )}
            ...
            {activeTool === 'TAX_EST' && (
                <div className="animate-fade-in">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold text-white tracking-widest font-orbitron">{t('tools.tax_estimator')}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mb-3">
                        <div>
                            <label className="text-[8px] text-slate-500 uppercase block mb-1">{t('tools.realized_gain')}</label>
                        ...
                        <div>
                            <label className="text-[8px] text-slate-500 uppercase block mb-1">{t('tools.tax_rate')}</label>
                    ...
                    {taxResult !== null && (
                        <div className="bg-brand-green/10 border border-brand-green/30 rounded-xl p-3 mb-3 text-center">
                            <p className="text-[9px] text-slate-400 uppercase font-mono">{t('tools.est_liability')}</p>
                            <p className="text-xl font-black text-brand-green font-orbitron drop-shadow-[0_0_8px_#00FF9D]">${taxResult.toFixed(2)}</p>
                            <p className="text-[9px] text-slate-500 mt-1 italic">{t('tools.nfa')}</p>
                        </div>
                    )}
