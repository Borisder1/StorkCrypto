...
export const PortfolioTools: React.FC = () => {
    ...
    // IL Calc State
    const [ilTokenA, setIlTokenA] = useState('0'); // % change
    const [ilTokenB, setIlTokenB] = useState('0'); // % change
    const [ilResult, setIlResult] = useState<number | null>(null);

    // Tax State
    const [taxRate, setTaxRate] = useState('19.5'); // Default UA
    ...
    return (
        <div className="bg-brand-card/40 border border-brand-border rounded-[2rem] p-5 mb-6 overflow-hidden shadow-xl">
            <div className="flex gap-2 mb-4 bg-black/40 p-1 rounded-xl overflow-x-auto no-scrollbar">
                <button onClick={() => { setActiveTool('SHIELD'); triggerHaptic('selection'); }} className={`flex-1 min-w-[80px] py-2 rounded-lg text-[9px] font-bold uppercase transition-all whitespace-nowrap ${activeTool === 'SHIELD' ? 'bg-brand-cyan text-black' : 'text-slate-500 hover:text-white'}`}>Wallet Shield</button>
                <button onClick={() => { setActiveTool('TIME_MACHINE'); triggerHaptic('selection'); }} className={`flex-1 min-w-[80px] py-2 rounded-lg text-[9px] font-bold uppercase transition-all whitespace-nowrap ${activeTool === 'TIME_MACHINE' ? 'bg-brand-cyan text-black' : 'text-slate-500 hover:text-white'}`}>Time Machine</button>
                <button onClick={() => { setActiveTool('IL_CALC'); triggerHaptic('selection'); }} className={`flex-1 min-w-[80px] py-2 rounded-lg text-[9px] font-bold uppercase transition-all whitespace-nowrap ${activeTool === 'IL_CALC' ? 'bg-brand-purple text-white' : 'text-slate-500 hover:text-white'}`}>DeFi Risk</button>
                <button onClick={() => { setActiveTool('TAX_EST'); triggerHaptic('selection'); }} className={`flex-1 min-w-[80px] py-2 rounded-lg text-[9px] font-bold uppercase transition-all whitespace-nowrap ${activeTool === 'TAX_EST' ? 'bg-brand-green text-black' : 'text-slate-500 hover:text-white'}`}>Tax Calc</button>
            </div>
                ...
                <div className="animate-fade-in">
                    ...
                    <div className="bg-black/30 rounded-xl p-4 border border-white/5 mb-3">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            {scanStatus === 'SCANNING' && <span className="text-[10px] text-yellow-500 animate-pulse">SCANNING...</span>}
                            {scanStatus === 'SAFE' && <span className="text-[10px] text-green-500 font-bold">ALL CLEAR</span>}
                        ...
                        <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                            <div className={`h-full transition-all duration-1000 ${scanStatus === 'SCANNING' ? 'w-full bg-yellow-500' : scanStatus === 'SAFE' ? 'w-full bg-green-500' : 'w-0'}`}></div>
                        </div>
                    </div>
                ...
                <div className="animate-fade-in">
                    <LockedFeature minLevel={5} title="FOMO SIMULATOR">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-xs font-bold text-white">FOMO SIMULATOR</span>
                        </div>
                        ...
                        <button onClick={calculateTimeMachine} className="w-full py-2 bg-white/5 border border-white/10 hover:bg-brand-cyan hover:text-black hover:border-brand-cyan transition-all rounded-xl text-[10px] font-bold uppercase">Calculate Potential</button>
                    </LockedFeature>
                </div>
            )}
                ...
                <div className="animate-fade-in">
                    <LockedFeature minLevel={10} title="IMPERMANENT LOSS">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-xs font-bold text-white">IMPERMANENT LOSS</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mb-3">
                            <div>
                                <label className="text-[8px] text-slate-500 uppercase block mb-1">Token A Change %</label>
                                <input type="number" value={ilTokenA} onChange={(e) => setIlTokenA(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg p-2 text-xs text-white outline-none" />
                            </div>
                            <div>
                                <label className="text-[8px] text-slate-500 uppercase block mb-1">Token B Change %</label>
                                <input type="number" value={ilTokenB} onChange={(e) => setIlTokenB(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg p-2 text-xs text-white outline-none" />
                            </div>
                        </div>

                        {ilResult !== null && (
                            <div className="bg-brand-purple/10 border border-brand-purple/30 rounded-xl p-3 mb-3 text-center">
                                <p className="text-[9px] text-slate-400 uppercase">Estimated Loss vs HODL</p>
                                <p className="text-xl font-black text-brand-purple font-orbitron">{ilResult.toFixed(2)}%</p>
                            </div>
                        )}

                        <button onClick={calculateIL} className="w-full py-2 bg-white/5 border border-white/10 hover:bg-brand-purple hover:text-white hover:border-brand-purple transition-all rounded-xl text-[10px] font-bold uppercase">Check Risk</button>
                    </LockedFeature>
                </div>
            )}
            ...
            {activeTool === 'TAX_EST' && (
                <div className="animate-fade-in">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold text-white">TAX ESTIMATOR</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mb-3">
                        <div>
                            <label className="text-[8px] text-slate-500 uppercase block mb-1">Realized Gain ($)</label>
                            <input type="number" value={realizedGain} onChange={(e) => setRealizedGain(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg p-2 text-xs text-white outline-none" placeholder="5000" />
                        </div>
                        <div>
                            <label className="text-[8px] text-slate-500 uppercase block mb-1">Tax Rate (%)</label>
                            <input type="number" value={taxRate} onChange={(e) => setTaxRate(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg p-2 text-xs text-white outline-none" placeholder="19.5" />
                        </div>
                    </div>

                    {taxResult !== null && (
                        <div className="bg-brand-green/10 border border-brand-green/30 rounded-xl p-3 mb-3 text-center">
                            <p className="text-[9px] text-slate-400 uppercase">Estimated Liability</p>
                            <p className="text-xl font-black text-brand-green font-orbitron">${taxResult.toFixed(2)}</p>
                            <p className="text-[9px] text-slate-500 mt-1 italic">Not financial advice.</p>
                        </div>
                    )}

                    <button onClick={calculateTax} className="w-full py-2 bg-white/5 border border-white/10 hover:bg-brand-green hover:text-black hover:border-brand-green transition-all rounded-xl text-[10px] font-bold uppercase">Calculate Tax</button>
                </div>
            )}
        </div>
    );
