        ...
        <div className="min-h-screen relative pb-40">
             ...
             <div className="px-5 pt-4 safe-area-pt relative z-20 max-w-md mx-auto">
                ...
                <div className="flex justify-between items-center mb-4 py-2 border-b border-white/5 backdrop-blur-sm bg-black/20 rounded-2xl px-3">
                    ...
                    <div className="flex items-center gap-3" onClick={() => window.location.reload()}>
                        <div className="relative group cursor-pointer">
                            <div className="w-8 h-8 rounded-xl bg-brand-cyan/5 border border-brand-cyan/20 flex items-center justify-center overflow-hidden transition-all group-hover:border-brand-cyan/50">
                                <img src={`${import.meta.env.BASE_URL}logo.jpg`} alt="StorkCrypto Logo" className="w-full h-full object-cover" />
                            </div>
                        ...
                        <div>
                            <h1 className="font-orbitron font-black text-[10px] sm:text-xs tracking-[0.15em] text-white leading-none">STORK<span className="text-brand-cyan">CRYPTO</span></h1>
                            <p className="text-[8px] text-slate-500 font-mono mt-0.5 uppercase tracking-wide">NEURAL TERMINAL</p>
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 bg-white/5 hover:bg-brand-cyan/10 border border-white/10 hover:border-brand-cyan/50 rounded-xl p-1.5 pr-3 transition-all group"
                    >
                        <div className="w-8 h-8 rounded-lg bg-black/50 flex items-center justify-center relative border border-white/5 overflow-hidden">
                             {userStats.avatarUrl ? (
                                 <img src={userStats.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                             ) : (
                                 <BotIcon className="w-4 h-4 text-slate-400 group-hover:text-brand-cyan transition-colors" />
                             )}
                             {/* Status Indicator */}
                ...
                >
                    ...
                    <div className="relative bg-[#0a0f1e]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-4 shadow-2xl overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-2 relative z-10">
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.4em] font-orbitron flex items-center gap-2">
                                <PieChartIcon className="w-3 h-3 text-brand-cyan" /> {t('portfolio.net_liquidity')}
                            </span>
                            <div className="w-6 h-6 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-slate-500 group-hover:text-brand-cyan transition-colors">
                                <ActivityIcon className="w-3 h-3" />
                            </div>
                        </div>
                        
                        <div className="flex items-baseline gap-2 mb-3 relative z-10">
                            <span className="text-2xl font-black text-white font-orbitron tracking-tighter drop-shadow-[0_0_20px_rgba(0,217,255,0.3)]">
                                <NumberTicker value={totalBalance} prefix="$" fractionDigits={0} />
                            </span>
                            <span className="text-[10px] font-black text-slate-600 font-mono">USD</span>
                        </div>
                ...
                >
                    <div className="relative group rounded-[2rem] bg-[#0a0f1e]/60 border border-brand-cyan/20 hover:border-brand-cyan/60 transition-all shadow-2xl h-24">
                        <button 
                            onClick={(e) => { e.stopPropagation(); openInfo('MINING'); }} 
                            className="absolute top-3 right-3 w-4 h-4 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-brand-cyan z-20"
                        >
                            <InfoIcon className="w-2.5 h-2.5" />
                        </button>
                        <button onClick={() => { triggerHaptic('medium'); setShowAirdrop(true); }} className="w-full h-full p-3 flex flex-col justify-between text-left relative z-10 overflow-hidden rounded-[2rem]">
                            <div className="absolute inset-0 bg-brand-cyan/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            <div className="w-8 h-8 rounded-xl bg-brand-cyan/10 border border-brand-cyan/30 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform"><PickaxeIcon className="w-4 h-4 text-brand-cyan" /></div>
                            <div><h3 className="font-orbitron font-black text-white text-[9px] uppercase tracking-widest">{t('home.mining')}</h3><p className="text-[7px] text-brand-cyan font-mono opacity-60">{t('home.earning_stork')}</p></div>
                        </button>
                    </div>

                    <div className="relative group rounded-[2rem] bg-[#0a0f1e]/60 border border-brand-purple/20 hover:border-brand-purple/60 transition-all shadow-2xl h-24">
                        <button 
                            onClick={(e) => { e.stopPropagation(); openInfo('SENTINEL'); }} 
                            className="absolute top-3 right-3 w-4 h-4 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-brand-purple z-20"
                        >
                            <InfoIcon className="w-2.5 h-2.5" />
                        </button>
                        <button onClick={() => { triggerHaptic('medium'); setShowSentinel(true); }} className="w-full h-full p-3 flex flex-col justify-between text-left relative z-10 overflow-hidden rounded-[2rem]">
                            <div className="absolute inset-0 bg-brand-purple/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            <div className="w-8 h-8 rounded-xl bg-brand-purple/10 border border-brand-purple/30 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform"><ShieldIcon className="w-4 h-4 text-brand-purple" /></div>
                            <div><h3 className="font-orbitron font-black text-white text-[9px] uppercase tracking-widest">{t('home.sentinel')}</h3><p className="text-[7px] text-brand-purple font-mono opacity-60">{t('home.auto_monitor')}</p></div>
                        </button>
                    </div>
                </motion.div>
                ...
                >
                    <button 
                        ...
                        className="w-full relative group rounded-[2rem] bg-gradient-to-r from-brand-cyan/10 to-brand-purple/10 border border-white/10 hover:border-brand-cyan/40 transition-all shadow-2xl p-3 flex items-center justify-between overflow-hidden"
                    >
                        <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                        <div className="flex items-center gap-3 relative z-10">
                            <div className="w-8 h-8 rounded-xl bg-black/40 border border-white/10 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform shrink-0">
                                <ZapIcon className="w-4 h-4 text-brand-cyan" />
                            </div>
                            <div className="text-left">
                                <h3 className="font-orbitron font-black text-white text-[10px] uppercase tracking-widest">{t('home.dex')}</h3>
                                <p className="text-[7px] text-slate-400 font-mono uppercase">{t('home.swap_protocol')}</p>
                            </div>
                        </div>
