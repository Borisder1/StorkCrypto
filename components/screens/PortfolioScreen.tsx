            ...
            <div className="flex items-center justify-between p-4 relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-mono mt-0.5">{asset.amount.toLocaleString()} {t('portfolio.units')}</p>
                    </div>
                </div>
...
const PortfolioScreen: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
    const { assets, updateAssetPrice, wallet, addAsset, syncChainAssets, settings } = useStore();
        ...
        <div className="fixed inset-0 z-[110] bg-brand-bg flex flex-col overflow-hidden animate-fade-in h-[100dvh] w-full">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 relative z-10 pt-6 pb-32">
                ...
                <div className="flex p-1 bg-black/40 rounded-xl border border-white/5 mb-6 shadow-inner mx-2">
                    <button onClick={() => setBalanceType('NEURAL')} className={`flex-1 py-2 rounded-lg text-[9px] font-black font-orbitron transition-all uppercase tracking-widest ${balanceType === 'NEURAL' ? 'bg-brand-cyan text-black shadow-lg' : 'text-slate-500'}`}>{t('portfolio.neural_demo')}</button>
                    <button onClick={() => setBalanceType('MAINNET')} className={`flex-1 py-2 rounded-lg text-[9px] font-black font-orbitron transition-all uppercase tracking-widest ${balanceType === 'MAINNET' ? 'bg-brand-green text-black shadow-lg' : 'text-slate-500'}`}>{t('portfolio.mainnet_ton')}</button>
                </div>
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div className="mt-4">
                            <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.3em] font-orbitron mb-2 flex items-center justify-center gap-2">
                                <PieChartIcon className="w-3 h-3 text-brand-cyan" /> {t('portfolio.net_worth')}
                            </p>
                ...
                {balanceType === 'NEURAL' && (useStore.getState().positions || []).length > 0 && (
                    <div className="px-2 mb-8">
                        <h3 className="text-[10px] font-black text-brand-cyan uppercase tracking-widest mb-3 ml-1 flex items-center gap-2">
                            <ActivityIcon className="w-3 h-3" /> {t('portfolio.active_positions')}
                        </h3>
                            ...
                            {(useStore.getState().positions || []).map(pos => (
                                <div key={pos.id} className="bg-brand-card/40 border border-white/5 rounded-2xl p-4 flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-[9px] text-slate-500 font-mono mt-1">{t('portfolio.entry')} ${pos.entryPrice.toFixed(2)}</p>
                                    </div>
                                    <div className="text-right flex items-center gap-4">
                                        <div>
                                            ...
                                            <p className="text-[8px] text-slate-500 font-mono">{t('portfolio.roi')} {pos.roi.toFixed(2)}%</p>
                                        </div>
                ...
                <div className="px-2">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">{t('portfolio.asset_allocation')}</h3>
                    <div className="space-y-1">
                        {isSyncing ? (
                            ...
                            <div className="py-12 text-center border border-dashed border-white/10 rounded-2xl bg-white/5 opacity-50 flex flex-col items-center">
                                ...
                                <p className="text-[10px] font-black font-orbitron uppercase tracking-widest text-slate-500">{t('portfolio.vault_empty')}</p>
                                <p className="text-[9px] font-mono text-slate-600 mt-1">{t('portfolio.add_assets_connect')}</p>
                            </div>
                        ) : (
                            (assets || []).map((asset) => <AssetEntry key={asset.ticker} asset={asset} totalPortfolioValue={totalValue} t={t} />)
                        )}
                    </div>
                </div>
            </div>
