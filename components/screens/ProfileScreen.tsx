...
const ProfileScreen: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
    ...
    const handleAvatarUpdate = (newAvatarUrl: string) => {
        ...
        updateUserStats({ avatarUrl: newAvatarUrl });
        localStorage.setItem('stork_user_avatar', newAvatarUrl);
        showToast(t('profile.identity_updated'));
    };
    ...
    const handleCopyAddress = () => {
        if (wallet.address) {
            ...
            showToast(t('profile.address_copied'));
        }
    };
    ...
    const handleXpRedeem = () => {
        ...
        if (userStats.xp < cost) {
            triggerHaptic('error');
            showToast(t('profile.insufficient_xp'));
            return;
        }
        if (window.confirm(t('profile.exchange_xp_confirm').replace('{cost}', cost.toString()))) {
            triggerHaptic('success');
            redeemXpForPro(1);
        }
    };
                    ...
                    <div className="relative z-10 p-6 flex flex-col items-center">
                        ...
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <div className="flex items-center gap-1 bg-green-900/20 px-1.5 py-0.5 rounded border border-green-500/20">
                                ...
                                <span className="text-[7px] font-black text-green-500 uppercase">{t('profile.cloud_sync')}</span>
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <div className="text-left">
                                ...
                                <span className="block text-[9px] font-mono text-white">
                                    {t('profile.xp_rate').replace('{rate}', (settings.xpToProRate || 500).toString())}
                                </span>
                            </div>
                        </button>
                    </div>
                ...
                <div className="bg-brand-card/40 border border-white/5 rounded-[2.5rem] p-6 mb-8 shadow-inner relative overflow-hidden">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <span className={`text-[9px] font-black px-2 py-1 rounded border ${subData.isTrial ? 'bg-green-500/10 border-green-500/30 text-green-400' : subData.isPaid ? 'bg-purple-500/10 border-purple-500/30 text-purple-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
                            {subData.isPaid ? t('profile.unlimited') : subData.isTrial ? t('profile.trial_mode') : t('profile.basic')}
                        </span>
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                ...
                                <p className={`text-[9px] font-mono uppercase mt-0.5 ${settings.twoFactorEnabled ? 'text-brand-green' : 'text-slate-500'}`}>
                                    {settings.twoFactorEnabled ? t('profile.active_secured') : t('profile.disabled_recommended')}
                                </p>
                            </div>
                        </div>
                ...
                <div className="bg-brand-card/40 border border-white/5 rounded-[2.5rem] p-6 mb-8 shadow-inner relative overflow-hidden">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                        ...
                                        <p className={`text-[9px] font-mono uppercase mt-0.5 ${settings.soundEnabled ? 'text-brand-cyan' : 'text-slate-500'}`}>
                                            {settings.soundEnabled ? t('profile.active') : t('profile.muted')}
                                        </p>
                                    </div>
                                </div>
                                <div className={`px-2 py-1 rounded text-[8px] font-black uppercase border ${settings.soundEnabled ? 'bg-brand-cyan/10 border-brand-cyan/30 text-brand-cyan' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
                                    {settings.soundEnabled ? t('profile.on') : t('profile.off')}
                                </div>
                            </button>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-col gap-2 bg-black/40 p-2 rounded-xl border border-white/5">
                                        ...
                                        if ('Notification' in window) {
                                            Notification.requestPermission().then(permission => {
                                                if (permission === 'granted') {
                                                    useStore.getState().showToast(t('profile.push_enabled'));
                                                }
                                            });
                                        } else {
                                            useStore.getState().showToast(t('profile.push_unsupported'));
                                        }
                                        triggerHaptic('selection');
                                    }} 
                                ...
                                <button 
                                    onClick={() => {
                                        triggerHaptic('selection');
                                        useStore.getState().showToast(t('profile.connecting_uniswap'));
                                        setTimeout(() => useStore.getState().showToast(t('profile.uniswap_connected')), 1500);
                                    }} 
                        ...
                        <div>
                            ...
                            <div className="flex bg-black/40 p-1 rounded-xl border border-white/5">
                                <button onClick={() => { updateSettings({ riskLevel: 'CONSERVATIVE' }); triggerHaptic('selection'); }} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${settings.riskLevel === 'CONSERVATIVE' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/50' : 'text-slate-500'}`}>{t('profile.conservative')}</button>
                                <button onClick={() => { updateSettings({ riskLevel: 'AGGRESSIVE' }); triggerHaptic('selection'); }} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${settings.riskLevel === 'AGGRESSIVE' ? 'bg-red-500/20 text-red-400 border border-red-500/50' : 'text-slate-500'}`}>{t('profile.aggressive')}</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div className={`p-5 rounded-3xl border transition-all ${wallet.isConnected ? 'bg-brand-cyan/5 border-brand-cyan/30' : 'bg-black/40 border-dashed border-white/10'}`}>
                             <div className="flex items-center justify-between mb-4">
                                 <div className="flex items-center gap-4">
                                     ...
                                     <div>
                                         <p className="text-white font-black text-xs uppercase tracking-widest">{wallet.isConnected ? (wallet.walletType || t('profile.connected_wallet')) : t('profile.no_wallet')}</p>
                                 ...
                                 >
                                     {wallet.isConnected ? t('profile.disconnect') : t('profile.initialize')}
                                 </button>
                             </div>
                             ...
                             {wallet.isConnected && wallet.address && (
                                <div className="bg-black/60 rounded-2xl p-4 border border-white/5 flex items-center justify-between cursor-copy active:bg-white/5 transition-colors" onClick={handleCopyAddress}>
                                    <div>
                                        <p className="text-[8px] text-slate-600 uppercase font-black mb-1">{t('profile.contract_address')}</p>
