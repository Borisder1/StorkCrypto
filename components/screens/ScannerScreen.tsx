...
const ScannerListItem = React.memo(({ coin, onAssetClick, style, isAlpha, t }: { 
    ...
    return (
        <div onClick={() => onAssetClick(coin)} style={style} className="absolute w-full px-1">
            <div className={`relative overflow-hidden rounded-xl h-[80px] transition-all cursor-pointer active:scale-[0.98] group ${isAlpha ? 'bg-brand-purple/10 backdrop-blur-md border-l-2 border-l-brand-purple border-y border-r border-white/10 shadow-[0_4px_15px_rgba(189,0,255,0.1)]' : 'bg-[#0a0f1e]/40 backdrop-blur-md border-b border-white/10 hover:bg-white/10 shadow-lg'}`}>
                ...
                <div className="relative z-10 flex items-center justify-between p-3 h-full">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                {isAlpha && <span className="text-[7px] font-black text-brand-purple bg-brand-purple/10 px-1 rounded animate-pulse">{t('scanner.alpha_tag')}</span>}
                            </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className={`flex flex-col items-center w-12 py-1 rounded bg-black/20 border border-white/5`}>
                            <span className="text-[6px] text-slate-500 font-black uppercase">{t('scanner.rsi')}</span>
        ...
        <div className="fixed inset-0 z-[120] bg-brand-bg flex flex-col animate-fade-in overflow-hidden h-[100dvh] w-full">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 p-4 space-y-6 pb-32">
                ...
                <div className="flex gap-2 bg-black/40 p-1.5 rounded-2xl border border-white/5 shadow-inner shrink-0">
                    <button onClick={() => { setActiveTab('MARKET'); }} className={`flex-1 py-3 rounded-xl text-[10px] font-black font-orbitron transition-all ${activeTab === 'MARKET' ? 'bg-brand-card text-brand-cyan border border-brand-cyan/20 shadow-xl' : 'text-slate-500'}`}>{t('scanner.list')}</button>
                    <button onClick={() => { setActiveTab('ALPHA'); }} className={`flex-1 py-3 rounded-xl text-[10px] font-black font-orbitron transition-all ${activeTab === 'ALPHA' ? 'bg-brand-card text-brand-cyan border border-brand-cyan/20 shadow-xl' : 'text-slate-500'}`}>{t('scanner.alpha')}</button>
            ...
            {showInfo && <InfoModal title={t('scanner.info_title')} description={t('scanner.info_desc')} features={[t('scanner.info_feat_1'), t('scanner.info_feat_2'), t('scanner.info_feat_3')]} onClose={() => setShowInfo(false)} />}
        </div>
    );
};
