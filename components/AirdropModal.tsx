            ...
            <div className="flex-1 w-full overflow-y-auto custom-scrollbar p-6 space-y-8 relative z-0">
                ...
                <div className="glass-panel border border-brand-cyan/30 rounded-[2.5rem] p-8 text-center relative overflow-hidden shadow-[0_0_30px_rgba(0,217,255,0.1)] group hover:shadow-[0_0_50px_rgba(0,217,255,0.2)] transition-shadow duration-500">
                    ...
                    <button 
                        onClick={handleClaim}
                        disabled={isClaiming}
                        className={`mt-6 w-full py-4 rounded-2xl font-black font-orbitron uppercase tracking-widest text-xs transition-all shadow-xl active:scale-90 active:shadow-none flex items-center justify-center gap-2 relative overflow-hidden ${minedAmount >= 0.1 ? 'bg-brand-cyan text-black hover:bg-white box-glow active:bg-brand-cyan/80' : 'bg-slate-800/50 text-slate-500 border border-white/5 cursor-not-allowed hover:bg-slate-800/70'}`}
                    >
                        {isClaiming ? (
                            <>
