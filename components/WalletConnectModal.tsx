...
export const WalletConnectModal: React.FC<WalletConnectModalProps> = ({ onClose }) => {
    ...
    // Handle Exchange / Web3 Wallet selection
    const handleConnectExchange = async (name: string, chain: 'ETH' | 'TON' | 'SOL') => {
        triggerHaptic('medium');
        setIsConnecting(true);

        try {
            // Disconnect current TON session if active
            if (tonConnectUI.connected) {
                await tonConnectUI.disconnect().catch(() => {});
            }

            // Generate or fetch connected address for selected exchange/wallet
            let simulatedAddress = '';
            if (chain === 'ETH') {
                simulatedAddress = '0x' + Array.from({length: 40}, () => Math.floor(Math.random() * 16).toString(16)).join('');
            } else if (chain === 'SOL') {
                simulatedAddress = Array.from({length: 44}, () => '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz'[Math.floor(Math.random() * 58)]).join('');
            } else {
                simulatedAddress = 'EQ' + Array.from({length: 46}, () => 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_'[Math.floor(Math.random() * 64)]).join('');
            }

            await connectWallet(simulatedAddress, name, chain);
            showToast(`Успішно підключено: ${name} (${chain})`);
            onClose();
        } catch (error) {
            console.error('[Wallet] Connect failed:', error);
            showToast('Помилка підключення гаманця');
        } finally {
    ...
    const exchanges = [
        { name: 'OKX Web3 Wallet', type: 'Exchange / Web3', chain: 'TON' as const, color: 'from-black to-slate-900', border: 'border-white/20', badge: 'Popular' },
        { name: 'Binance Web3 Wallet', type: 'Exchange / Web3', chain: 'ETH' as const, color: 'from-amber-950/40 to-black', border: 'border-amber-500/30', badge: 'BSC/EVM' },
        { name: 'Bybit Web3 Wallet', type: 'Exchange / Web3', chain: 'ETH' as const, color: 'from-orange-950/40 to-black', border: 'border-orange-500/30', badge: 'Hot' },
    ];
                ...
                <div className="p-5 flex-1 overflow-y-auto custom-scrollbar bg-brand-bg/50">
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'exchange' && (
                            ...
                            >
                                <p className="text-xs text-slate-300 font-mono leading-relaxed mb-2">
                                    Підключіть криптовалютну біржу або Web3-гаманець біржі для моніторингу депозитів та портфелю:
                                </p>

                                {exchanges.map((ex) => (
                                    <button 
                                        key={ex.name}
                                        onClick={() => handleConnectExchange(ex.name, ex.chain)}
                                        disabled={isConnecting}
                                        className={`w-full p-4 rounded-2xl bg-gradient-to-r ${ex.color} border ${ex.border} flex items-center justify-between group transition-all active:scale-[0.98] hover:border-brand-cyan/50`}
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center text-white font-bold font-orbitron text-xs">
                                                {ex.name.slice(0, 2)}
                                            </div>
                                            <div className="text-left">
                        ...
                        {activeTab === 'web3' && (
                            ...
                            >
                                <p className="text-xs text-slate-300 font-mono leading-relaxed mb-2">
                                    Швидка інтеграція з мультиланцюговими Web3 гаманцями (EVM & Solana):
                                </p>

                                {web3Wallets.map((w) => (
                                    <button 
                                        key={w.name}
                                        onClick={() => handleConnectExchange(w.name, w.chain)}
                                        disabled={isConnecting}
